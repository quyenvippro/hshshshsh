...
async function checkupdate() {
	try {
		...
		if (data.version != global.package.version) {
			console.log("➥ 🆙 Đã có bản cập nhật mới (ò_ó)");
		} else {
		console.log("⚜️ 《Bạn đang sử dụng phiên bản mới nhất ('･ω･´) 》⚜️");
		}
	} catch(e) {
		console.log("➥ 🛸 Đã có lỗi xảy ra rồi :<.");
		console.log(e)
	}
}
checkupdate()
bot.on('message', function (ctx) {
    if(ctx.message.text != undefined && ctx.message.text.slice(0,global.config.prefix.length) == global.config.prefix){
        ...
        if(ctx.message.photo){
            ...
            console.log("\x1b[K" + "\x1b[1;32m" + "\x1b[1;92m" + "\x1b[38;2;0;255;0m" + "▐[" + (date.getUTCFullYear()+ "-" + (date.getUTCMonth() + 1)+ "-" + date.getUTCDate()+ "T" + date.getUTCHours()+ "-" + date.getUTCMinutes()+ "-" + date.getUTCSeconds()+ "." + date.getUTCMilliseconds()+ "Z") + "]" + " " + `\n▐[${ctx.message.from.id} đến ${ctx.message.chat.id}]\n▐[Icon của sticker : ${ctx.message.sticker.emoji}]`);
        } else {
    console.log("\x1b[K" + "\x1b[1;32m" + "\x1b[1;92m" + "\x1b[38;2;0;255;0m" + "  ┌ [🚦 " + (date.getUTCFullYear()+ "-" + (date.getUTCMonth() + 1)+ "-" + date.getUTCDate()+ "T" + date.getUTCHours()+ "-" + date.getUTCMinutes()+ "-" + date.getUTCSeconds()+ "." + date.getUTCMilliseconds()+ "Z") + "]" + " " + `\n〄│ [👥 Người dùng @${ctx.message.from.username} (${ctx.message.from.id}) \n  │ [📫 Nhắn đến nhóm ${ctx.message.chat.title} (${ctx.message.chat.id})]\n  └ [💬 Tin nhắn : ${ctx.message.text}]`);
        }})
function runCM(ctx){
    ...
    if (!ccm) ctx.reply(`╔😼 Sai lệnh\n║✨ Sử dụng lệnh "${global.config.prefix}help" 💵\n║❤️ Để xem danh sách lệnh nha ツ\n╚👩‍💻 Bot by : @lisa_is_me`);
}
...
function loadPlugin(){
    ...
    for(var i=0; i<listFile.length; i++){
        try{
            ...
            console.error("➥ ⛔ Không thể load file [\""+listFile[i]+"\"] với lỗi: » "+err)
        }
    }
}
    ...
    if(typeof pluginInfo.nodeDepends == "object"){
        for (var i in pluginInfo.nodeDepends){
            if (!fs.existsSync(path.join(__dirname, "node_modules", i, "package.json"))) {
                ...
                console.log("➥ ⚙️ Tiến hành cài đặt Module \""+i+"\" cho Func \""+pluginInfo.name+"\":\n");
...
if(!global.config.token){
	...
	console.log(" __        ______   ______    ______  \n/  |      /      | /      \  /      \ \n$$ |      $$$$$$/ /$$$$$$  |/$$$$$$  |\n$$ |        $$ |  $$ \__$$/ $$ |__$$ |\n$$ |        $$ |  $$      \ $$    $$ |\n$$ |        $$ |   $$$$$$  |$$$$$$$$ |\n$$ |_____  _$$ |_ /  \__$$ |$$ |  $$ |\n$$       |/ $$   |$$    $$/ $$ |  $$ |\n$$$$$$$$/ $$$$$$/  $$$$$$/  $$/   $$/ ");
    console.log("\n")
  console.log("👑 𝙼ọ𝚒 𝚝𝚑ắ𝚌 𝚖ắ𝚌 𝚟𝚞𝚒 𝚕ò𝚗𝚐 𝚕𝚒ê𝚗 𝚑ệ 𝚃𝚎𝚕𝚎𝚐𝚛𝚊𝚖 : @𝚕𝚒𝚜𝚊_𝚒𝚜_𝚖𝚎 👑")
  console.log("\n")
  console.log("✨ This Project made by JustGon ✨")
    console.log("\n")
    bot.launch()
}
